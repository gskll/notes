...
def tsp(cities: list[int], paths: list[list[int]], dist: int) -> bool:
    path_permutations = permutations(cities)
    for curr_path in path_permutations:
        curr_dist = 0
        for step in range(1, len(curr_path)):
            curr_dist += paths[curr_path[step - 1]][curr_path[step]]
